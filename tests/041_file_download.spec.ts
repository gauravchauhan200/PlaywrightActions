...
test('File Download',async({page})=>{
    ...
    const [download] = await Promise.all(
        [
            ...
            page.locator('button[onclick="downloadFile()"]').click(),
        ]
    )
    expect(download.suggestedFilename()).toContain("sample.txt");

    // save te file in the custom path
    // const downloadPath = "downloads/sample.txt" //static name 
    ...
    // delete the file /clean up the file in the folder
...
// Approach 2- using our own browser context
test('Open PDF2',async({ page })=>{
